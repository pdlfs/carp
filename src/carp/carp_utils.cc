...
int PivotUtils::CalculatePivots(CarpBase* carp, const size_t num_pivots) {
  carp->mutex_.AssertHeld();

  assert(num_pivots <= CarpBase::kMaxPivots);
  ...
  logv(__LOG_ARGS__, LOG_DBG2, "pvt_calc_local @ R%d, pvt width: %.2f\n", carp->my_rank_,
...
int PivotUtils::CalculatePivotsFromOob(CarpBase* carp, int num_pivots) {
  carp->mutex_.AssertHeld();
  ...
  carp->GetPartitionedOOBProps(oobl, oobr);
 ...
 * */
int PivotUtils::CalculatePivotsFromAll(CarpBase* carp, int num_pivots) {
  carp->mutex_.AssertHeld();
  ...
  fprintf(stderr,
          ...
          "rbc: %s (%zu)\n"
          "bin: %s (%zu)\n",
          carp->my_rank_, oobl_sz, oobr_sz, range_start, range_end,
...
int PivotUtils::GetRangeBounds(CarpBase* carp, std::vector<float>& oobl,
