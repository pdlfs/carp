...
void PartitionManifestReader::ReadFooterEpoch(int epoch, int rank, Slice& data,
  ...
  for (int i = 0; i < num_items; i++) {
    ...
    item.expected.range_min = DecodeFloat32(&data[cur_offset + offsets_[2]]);
    item.expected.range_max = DecodeFloat32(&data[cur_offset + offsets_[3]]);
    item.observed.range_min = DecodeFloat32(&data[cur_offset + offsets_[4]]);
    item.observed.range_max = DecodeFloat32(&data[cur_offset + offsets_[5]]);
